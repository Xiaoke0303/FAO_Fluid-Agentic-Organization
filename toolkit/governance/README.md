...
`toolkit/governance/` 继续保留，主要作为旧治理实现来源、历史分组与迁移来源之一。当前理解仓库整体结构的首选入口应是 `framework/`。
...
- `framework/` 是当前新的上位骨架候选
- `governance/` 中的内容可作为新框架 Assurance 模块的来源、映射对象或历史实现参考
- 后续以 `framework/MIGRATION-PLAN.md` 为准逐步迁移
...
1. 先看 `framework/UNIVERSAL-WORK-NODE-FRAMEWORK.md` 理解新骨架
2. 再看 `framework/TRUTH-CONTRACT.md`、`framework/OPENCLAW-MAPPING.md` 理解 Assurance 层

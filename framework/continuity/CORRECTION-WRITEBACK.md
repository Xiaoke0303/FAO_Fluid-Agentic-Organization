...
| **`MEMORY-INDEX.md`（摘要写回）** | 成本约束触发降级，写入核心摘要，附来源引用，内容摘要可供后续回查 |
| **`MEMORY-INDEX.md`（延后写回）** | 成本约束触发延后，标记延后写回，附触发原因、来源引用与原始内容摘要 |
...
摘要写回与延后写回不改变纠错本身的分型和修正表达，只改变写入 MEMORY-INDEX 的形式与时机。

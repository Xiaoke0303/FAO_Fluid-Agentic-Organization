...
> 本章节定义 FAO 核心概念的最小关系结构。不新增概念，只整理已有概念的关系。

### 概念分层

| 层级 | 概念 | 定义 | 权威来源 |
|------|------|------|----------|
| 本体层 | 主体（Actor） | 能够作为行动节点存在的实体 | 白皮书第2章 |
| 本体层 | 身份（Identity） | 系统区分"是谁在行动"的标识 | `IDENTITY.md` |
| 关系层 | 角色（Role） | 该主体在当前组织结构中占据的职责位置 | `ROLE-CONTRACT.md` |
| 关系层 | 能力（Capability） | 该主体事实上能够做什么 | Skill 商品化分析（笔记） |
| 关系层 | 管辖范围（Jurisdiction） | 该主体在哪些任务、资源、判断范围内拥有有效作用域 | 白皮书第3章 |
| 关系层 | 授权（Authority） | 谁赋予该主体进行某项行为的合法组织权限 | `EXTERNAL-CALL-PROTOCOL.md` |
| 关系层 | 委托（Delegation） | 拥有权力的主体将其中一部分交给其他主体的条件性转移 | 白皮书第4章 |
| 关系层 | 责任（Responsibility） | 行为结果最终如何回到组织责任结构 | `CONSTITUTION.md` |
| 工程层 | 凭证（Credential） | 身份/权限的技术证明方式 | `EXTERNAL-CALL-PROTOCOL.md` §External Write Gate |
...
1. **能力独立于授权** — 主体可以有能力做某事，但没有被授权做某事
2. **有身份不等于有角色** — 身份是跨任务的，角色是任务特定的
3. **有角色不等于有全部管辖权** — 角色只覆盖特定职责范围
4. **授权可附带条件和时效** — 授权不是无条件的永久许可
5. **委托不等于责任转移** — 委托只转移执行权，不转移最终责任
6. **凭证只证明，不定义** — 凭证是身份/权限的技术证明，不是治理本体概念

### 与框架模块的映射

| 概念 | 主要承载模块 |
|------|-------------|
| 主体/身份 | Role（`IDENTITY.md`） |
| 角色/收口资格 | Role（`ROLE-CONTRACT.md`） |
| 授权/凭证/外部边界 | Assurance + Runtime（`EXTERNAL-CALL-PROTOCOL.md`） |
| 委托/路由 | Runtime（`OPERATING-RULES.md`） |
| 责任/宪法约束 | Constitution（`CONSTITUTION.md`） |

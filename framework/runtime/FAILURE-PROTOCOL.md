...
失败报告记录发生了什么。失败熔断改变后续可执行空间。

### 熔断规则

1. **参数/schema 错误后回验**
   - 因参数错误、schema 不匹配、required field 缺失导致的失败，第 1 次发生后必须停止盲试。
   - 必须先回到 schema 验证（参照 [`EXTERNAL-CALL-PROTOCOL.md`](EXTERNAL-CALL-PROTOCOL.md) 的 Tool Schema Gate），确认参数正确后再尝试。
   - 不得在 schema 未验证的情况下直接重试。

2. **同类失败 2 次后禁止第 3 次盲冲**
   - 同一工具或同一目标连续失败 2 次后，禁止第 3 次盲目重试。
   - 必须执行：标记 `repeated_failure`；最小排查；暂停或升级。
   - 此规则与 Repeated Failure Rule 一致，但 Failure Latch 进一步要求：失败后必须收敛，不得扩大范围。

3. **失败后不得自动扩大范围**
   - 失败后不得自动扩大历史读取范围（"为了找原因多读一些"）。
   - 失败后不得自动扩大权限范围（"可能需要更多权限"）。
   - 失败后不得自动扩大外部调用范围（"换另一个工具试试"）。
   - 必须先收敛问题，再决定是否升级或写回。

4. **先收敛，再决定**
   - 失败后默认动作是收敛：停止继续展开，输出当前已积累的判断。
   - 收敛后，再决定：是否升级给人、是否写回、是否暂停。
   - 不得为了"推进"而扩大上下文、权限或调用范围。

### failure report vs failure latch

| 维度 | Failure Report | Failure Latch |
|------|----------------|---------------|
| 作用 | 记录 | 改变可执行空间 |
| 输出 | 失败报告对象 | 熔断声明 + 下一步 |
| 时机 | 失败后立即 | 失败后决定是否继续 |

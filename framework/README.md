...
FAO 的问题起点是组织治理，不是 agent 编排。但组织协作不是从"多节点流动"开始，而是从"单节点是否可治理"开始。

白皮书说明为什么责任不能简单随任务流动；framework 定义的是任务开始流动之前，每个工作节点至少需要具备的可治理结构。如果一个节点不能说明自己的角色、边界、真实性状态、纠错机制和节律，多节点协作只会放大责任扩散。因此 framework 不是多 agent 编排方案，而是可治理工作节点的最小结构定义。

...
**基础设施层**
- 模型层：OpenAI/Anthropic/DeepSeek/通义...
- Agent 平台层：Codex/Cursor/Claude Code/TRAE...
- 知识与上下文层：代码库/文档库/规范库/历史记录/提交日志
- 工具与集成层：Git/Jira/Confluence/测试平台...

**执行层 — Agent 编排 + 失败升级通道**

Agent 协同体系：
- PM Agent：需求拆解/任务规划/文档
- SA Agent：架构设计/技术选型/评估
- Coding Agent：代码生成/重构/修复
- Review Agent：代码评审/规范/缺陷 → **人类终审定**
- QA Agent：测试设计/执行/分析 → **人类抽样确认**
- Ops Agent：部署/监控/告警/故障

Agent 编排与协同：任务分发/状态管理/上下文共享/结果聚合

🚨 **失败升级通道**：Agent 失败/超时/异常/越界 → 自动暂停 → 路由人类

**流程层 — 每个环节都有'人类锚点'**

| 环节 | 人类锚点 | Agent 可辅助 |
|------|---------|-------------|
| 需求与规划 | 需求签字确认 | 收集/拆解/整理 |
| 设计与规划 | 架构决策人定 | 方案生成/选型 |
| 编码与实现 | 关键模块人审 | 代码生成/重构/文档 |
| 测试与验证 | **双重验证：人类抽样确认** | QA Agent 执行 |
| 核验与评审 | **双重验证：人类终审定** | Review Agent 初筛 |
| 发布与运维 | **🚪 人类强制确认门** | 部署/监控/告警 |
...
3. 效率指标
4. 质量指标
5. 业务价值指标（Intent / Efficiency）

能力模型（责任锚定型）：
- L1 业务工程师：定义问题、业务语境判断
- L2 提示词工程师：意图工程、上下文设计
- L3 验证工程师：人类验证 Agent 产出、抽样复核 Review Agent（新增角色）
- L4 技术经理：多 Agent 管理、架构决策（人定）、ROI 与价值管理
- L5 团队经理：研发体系设计、治理与风险管理、对外责任承担

**治理层 — 可流动的是流程，不可流动的是责任**
- 模型治理：版本/提示词/效果监控
- 代码治理：归属/版权/合规/扫描
- 安全治理：防护/敏感信息/审计
- 成本治理 ⭐：Token → 价值比、预算/配额/ROI
- Agent 治理：权限/策略/审计日志
- 质量治理：标准/评估/监控
- 策略与制度：伦理规范/数据隐私/安全合规/责任审查
